  ...
  template: `
    <div *ngIf="showIconLoader(); else dashboard" class="flex h-dvh items-center justify-center bg-zinc-950 px-6" role="status" aria-live="polite">
      <section class="w-full max-w-sm text-center">
        ...
        <p *ngIf="!state().gameRunning" class="mt-3 text-sm text-zinc-500">(Start the game to continue)</p>
                  ...
                  <ng-container *ngIf="currentDamageDone() as damage; else noDamageDone">
                    ...
                    <div *ngIf="damageDoneEntries().length; else noDamageEntries" class="divide-y divide-zinc-800">
                      <div *ngFor="let entry of damageDoneEntries(); trackBy: trackDamageEntry" class="flex items-center gap-3 px-2 py-2 text-sm hover:bg-zinc-800/60">
                        ...
                        <div class="min-w-0 flex-1"><p class="truncate text-zinc-200">{{ $any(entry).englishName || $any(entry).name || 'Unknown source' }}</p><p class="text-[11px] text-zinc-600">{{ compactNumber($any(entry).damage) }} damage<span *ngIf="$any(entry).castCount != null"> | Casts: {{ $any(entry).castCount }}</span><span *ngIf="$any(entry).hitCount != null"> | Hits: {{ $any(entry).hitCount }}<span *ngIf="$any(entry).criticalCount != null && $any(entry).missCount != null"> (crit: {{ $any(entry).criticalCount }}, miss: {{ $any(entry).missCount }})</span></span></p></div>
